...
fields = c('chr_beg_end_str','type', 'gene','biotype', 'intern', 'level', 'alt')

colnames(data) = c(paste(fields, '1', sep='.'), paste(fields, '2', sep='.'), 'status')

data = subset(data, level.1==1 & level.2==1 & biotype.1!= 'other' & biotype.2!= 'other' & status==STATUS_121)
...
func1  <- function(data, class, species) {
    ...
    colnames(res) = c('Segment','Biotype','Prop','SE')

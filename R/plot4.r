...
data$Segment = factor_segment(data$Segment)
...
a <- ggplot(data, aes(x = Segment, y = Prop, fill=Probability))
a <- a + geom_bar(stat = "identity", position = "dodge") + geom_errorbar(aes(ymin=LB, ymax=UB),width=.5,position=position_dodge(.9)) + facet_grid(Species ~ Biotype)
